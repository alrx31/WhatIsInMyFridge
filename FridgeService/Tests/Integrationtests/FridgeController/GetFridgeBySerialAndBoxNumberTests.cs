...
{
    public class GetFridgeBySerialAndBoxNumberTests:ControllerTests 
    {
        ...
        [Fact]
        public async Task GetFridgeBySerialAndBoxNumber_Success_WhenFridgeExist()
        {
           ...
           var responseAdd = await _httpClient.SendAsync(new HttpRequestMessage(new HttpMethod("PUT"), $"/api/fridge")
           {
            ...
            var request = new HttpRequestMessage(new HttpMethod("GET"), $"/api/fridge/serial/{fridge.serial}/{fridge.boxNumber}");
