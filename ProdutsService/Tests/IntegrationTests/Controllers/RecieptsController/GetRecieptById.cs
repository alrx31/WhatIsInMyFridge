...
{
    public class GetRecieptById : ControllerTests
    {
        [Fact]
        public async Task GetRecieptById_Success_ShouldReturnReciept()
        {
            var faker = new Faker();
            ...
            // get reciept by id
            // Act
            var response = await _httpClient.SendAsync(new HttpRequestMessage(new HttpMethod("GET"), $"/api/reciepts/{reciepts[0].Id}"));
