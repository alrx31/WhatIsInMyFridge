...
public class DeleteProductFromRecieptTests : ControllerTests
{
    [Fact]
    public async Task DeleteProductFromReciept_Success_ShouldDeleteProduct()
    {
        var faker = new Faker();
        ...
        var delPrrequest = new DeleteProductFromRecieptDTO
        {
            ProductId = pr.Id,
            RecieptId = reciepts[0].Id
        };
        ...
        var response = await _httpClient.SendAsync(
            new HttpRequestMessage(new HttpMethod("DELETE"), "api/Reciepts/reciept/products")
            {
                Content = new StringContent(
                    JsonSerializer.Serialize(delPrrequest),
                    Encoding.UTF8,
                    "application/json"
                )
            });
