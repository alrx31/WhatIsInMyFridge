...
{
    public class DeleteReciept : ControllerTests
    {
        [Fact]
        public async Task DeleteReciept_Success_ShouldDeleteReciept()
        {
            var faker = new Faker();
            ...
            // get reciept by id
            // Act
            var response = await _httpClient.SendAsync(new HttpRequestMessage(new HttpMethod("DELETE"), $"/api/reciepts/{reciepts[0].Id}"));
