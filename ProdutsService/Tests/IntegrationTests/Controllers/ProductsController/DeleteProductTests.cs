...
{
    public class DeleteProductTests : ControllerTests
    {
        [Fact]
        public async Task DeleteProduct_Success_ShouldDeleteProduct()
        {
            var faker = new Faker();
            ...
            var response = await _httpClient.SendAsync(new HttpRequestMessage(new HttpMethod("DELETE"), $"/api/products/{products[0].Id}"));
